...
// *********************************************************
    ...
    GLint bIsWaterObject_UniformLocation = glGetUniformLocation(shaderID, "bIsWaterObject");
    GLint bIsCrystalObject_UniformLocation = glGetUniformLocation(shaderID, "bIsCrystalObject");
    // HACK: I'm checking to see if the texture matches the flame object
    if ( pCurrentMeshObject->textures[0] == "flame.bmp" )
    {
...
//        glDepthFunc(GL_LESS); // We'll talk about this when we talk about the stencil buffer
    ...
    if (pCurrentMeshObject->textures[0] == "water.bmp")
    {
        glUniform1f(bIsWaterObject_UniformLocation, (GLfloat)GL_TRUE);
        //        glDisable(GL_DEPTH_TEST);
        //        glDepthFunc(GL_NEVER); // We'll talk about this when we talk about the stencil buffer
        glDepthMask(GL_FALSE);      // DON'T write to the depth buffer
    }
    ...
    {
        glUniform1f(bIsWaterObject_UniformLocation, (GLfloat)GL_FALSE);
        //        glDepthFunc(GL_LESS); // We'll talk about this when we talk about the stencil buffer
        glDepthMask(GL_TRUE);      // DON'T write to the depth buffer
    }
    if (pCurrentMeshObject->meshName == "Crystal")
    {
        glUniform1f(bIsCrystalObject_UniformLocation, (GLfloat)GL_TRUE);
        //        glDisable(GL_DEPTH_TEST);
        //        glDepthFunc(GL_NEVER); // We'll talk about this when we talk about the stencil buffer
        glDepthMask(GL_FALSE);      // DON'T write to the depth buffer
    }
    ...
    {
        glUniform1f(bIsCrystalObject_UniformLocation, (GLfloat)GL_FALSE);
        //        glDepthFunc(GL_LESS); // We'll talk about this when we talk about the stencil buffer
        glDepthMask(GL_TRUE);      // DON'T write to the depth buffer
    }
// *********************************************************bIsWaterObject
