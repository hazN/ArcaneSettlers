...
//#define PRINT_DEBUG_INFO
		...
		for (int i = 0; i < gameObjects.size(); i++)
		{
			...
			else if (go->Animation.AnimationType.length() != 0)
			{
				...
				if (itFind != m_Animations.end())
				{
					...
					go->mesh->position = GetAnimationPosition(itFind->second, animation.AnimationTime);
					go->mesh->scaleXYZ = GetAnimationScale(itFind->second, animation.AnimationTime);
					go->mesh->qRotation = GetAnimationRotation(itFind->second, animation.AnimationTime);
	...
	void AnimationManager::LoadAssimpAnimation(CharacterAnimationData& characterAnimation, const aiAnimation* animation, BoneHierarchy* boneHierarchy)
	{
