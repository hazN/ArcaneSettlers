...
uniform mat4 instanceModelMatrices[100];
...
void main()
{
    ...
    {
        // Use instancing if its a particle
        if (bIsInstanced)
        {
            gl_Position = (mProjection * mView * instanceModelMatrices[gl_InstanceID]) * vec4(vertPosition.xyz, 1.0f);
        }
        else
        {
